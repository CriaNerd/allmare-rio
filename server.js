...
function meHeaders(){return {Authorization:'Bearer '+ME_TOKEN,'Content-Type':'application/json',Accept:'application/json','User-Agent':'Allmare ecommerce contato@allmare.com.br'};}
async function meRequest(endpoint,options={}){const r=await fetch(ME_BASE+endpoint,{...options,headers:{...meHeaders(),...(options.headers||{})}});const text=await r.text();let data={};try{data=text?JSON.parse(text):{};}catch{data={raw:text};}if(!r.ok)throw new Error(data.message||data.error||`Melhor Envio respondeu ${r.status}`);return data;}
...
async function quoteShipping(cep,qty=1){
  if(!ME_TOKEN||!process.env.MELHOR_ENVIO_FROM_POSTAL_CODE)throw new Error('Integração de frete não configurada');
...
const server=http.createServer(async(req,res)=>{
 try{
  ...
  if(url.pathname.startsWith('/api/admin/')){
    ...
    if(req.method==='GET'&&labelMatch){const id=decodeURIComponent(labelMatch[1]);const o=(await dbListOrders()).find(x=>x.id===id);if(!o||!o.label_url)return json(res,404,{error:'Etiqueta ainda não disponível'});const label=await fetch(o.label_url);if(!label.ok)return json(res,502,{error:'Não foi possível baixar a etiqueta'});const bytes=Buffer.from(await label.arrayBuffer());res.writeHead(200,{'Content-Type':'application/pdf','Content-Disposition':`attachment; filename="etiqueta-${id}.pdf"`,'Content-Length':bytes.length,'Cache-Control':'no-store'});return res.end(bytes);}
...
try{
  await initDatabase();
  server.listen(PORT,()=>console.log(`Allmare V22 funcionando em http://localhost:${PORT} | banco: ${usingPostgres?'PostgreSQL':'JSON local (fallback)'}`));
}catch(e){
